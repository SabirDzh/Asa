...
class _TaskDetailSheet extends StatelessWidget {
  ...
  String _folderName(BuildContext context) {
    ...
    final folder = Provider.of<TaskProvider>(context, listen: false)
        .folders
        .firstWhere((f) => f.id == task.folderId, orElse: () => FolderItem(id: '', name: ''));
    return folder.name.isNotEmpty ? folder.name : settings.tr('no_folder');
  }

  String _statusText(BuildContext context) {
    final settings = Provider.of<SettingsProvider>(context, listen: false);
    return task.isCompleted ? settings.tr('status_completed') : settings.tr('status_active');
  }
  ...
  String _timeInfo(BuildContext context) {
    ...
    if (task.expectedDuration != null) {
      buffer.write('${settings.tr('duration')}: ${_formatDuration(task.expectedDuration!)}');
    }
  ...
  Future<void> _confirmDelete(BuildContext context) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        backgroundColor: bg,
        title: Text(settings.tr('confirm_delete_title'),
            style: TextStyle(color: isDark ? AppColors.textDark : AppColors.textLight)),
        content: Text(settings.tr('confirm_delete_content'), style: TextStyle(color: text)),
        actions: [
          TextButton(
        ...
        child: SafeArea(
          child: SingleChildScrollView(
            child: Column(
              ...
              const SizedBox(height: 20),
              _infoTile(Iconsax.clipboard_tick, task.title, textColor, bold: true),
              ...
              _infoTile(Iconsax.calendar_1, '${settings.tr('created_at')}: ${_formatDateTime(task.createdAt)}', textSecondary),
              _infoTile(Iconsax.refresh, '${settings.tr('updated_at')}: ${_formatDateTime(task.updatedAt)}', textSecondary),
              ...
              if (!_isInStreakFolder)
                if (task.calendarEventId != null)
                  _actionTile(Iconsax.calendar_remove, settings.tr('remove_from_calendar'), () {
                    context.read<TaskProvider>().unlinkTaskFromCalendar(task.id);
                    Navigator.pop(context);
                  }, textColor)
                else
                  _actionTile(Iconsax.calendar, settings.tr('add_to_calendar'), () {
                    Navigator.pop(context);
                    _showCalendarPicker(context);
                  }, textColor),
              _actionTile(Iconsax.trash, settings.tr('delete'), () => _confirmDelete(context), Colors.red),
  ...
  Widget _infoTile(IconData icon, String text, Color color, {bool bold = false}) {
    return Padding(
  ...
  Future<void> _showCalendarPicker(BuildContext context) async {
      ...
      selected = await showDialog<Calendar>(
        ...
        builder: (ctx) {
          ...
          return AlertDialog(
            ...
            title: Text(settings.tr('calendar_select'),
                style: TextStyle(color: isDark ? AppColors.textDark : AppColors.textLight)),
            content: SizedBox(
    ...
    await context.read<TaskProvider>().linkTaskToCalendar(task.id, selected.id!, date);
  }
}
